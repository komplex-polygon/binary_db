...
int log_value(u_int_index a, user_db b) {
  ...
  for (unsigned int bindd = 0; bindd < a.len; bindd++) {
    if (a.bin[bindd].ac == 1) {
      printf("%u\n0[a:%u,e:%u,q:%u]\n1[a:%u,e:%u,q:%u]\nhi:%u,de:%u]\n\n",
             bindd, a.bin[bindd].a0 - 1, (unsigned int)a.bin[bindd].e0,
             a.bin[bindd].q0, a.bin[bindd].a1 - 1,
             (unsigned int)a.bin[bindd].e1, a.bin[bindd].q1, a.bin[bindd].hi,
             a.bin[bindd].de);
    }
  }
...
unsigned int new_u_int_index(u_int_index *a, unsigned int id,
                             unsigned int path) {
...
int main() {
  ...
  */
  ...
  for (unsigned int yyu = 0; yyu < 1024 * 128; yyu++) {
    ...
    // strncpy(b.bin[test - 1].un, "Hello world.", sizeof(b.bin[test - 1].un) -
    // 1); b.bin[test - 1].un[sizeof(b.bin[test - 1].id) - 1] = '\0';
